...
/**
 * Reenvía el mensaje al webhook de la API Agente (mismo contrato que BuilderBot Cloud).
 */
export async function forwardToAgente(ctx, provider, { publicBaseUrl }) {
  ...
  const apiUrl = (process.env.AGENTE_API_URL || "http://localhost:4000").replace(/\/$/, "");
  ...
  if (!res.ok) {
    console.error("[cleexs-wa-bot] API error", res.status, parsed);
    throw new Error(parsed.error || `Agente API respondió ${res.status}`);
  }
